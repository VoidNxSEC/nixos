...

{
  ...
  options.kernelcore.security.compiler-hardening.enable =
    lib.mkEnableOption "fortified compiler flags"
    // {
      default = true;
    };

  config = lib.mkIf config.kernelcore.security.compiler-hardening.enable {
    ##########################################################################

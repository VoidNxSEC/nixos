...
in
{
  ...
  options.kernelcore.shell.aliases.docker-build.enable =
    lib.mkEnableOption "docker build aliases"
    // {
      default = true;
    };

  config = lib.mkIf config.kernelcore.shell.aliases.docker-build.enable {

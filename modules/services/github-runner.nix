...
let
  ...
  commonServiceOverrides = {
    ...
    BindReadOnlyPaths = [
      ...
      "${config.kernelcore.system.user.homeDir}/.ssh"
    ];
  };
  ...
  */
  ...
  mkEnvOverride = name: {
    "github-runner-${name}" = {
      environment.GIT_SSH_COMMAND = "ssh -i ${config.kernelcore.system.user.homeDir}/.ssh/id_ed25519 -o StrictHostKeyChecking=accept-new";
    };
  };

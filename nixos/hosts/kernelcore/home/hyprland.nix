...

{
  ...
  programs.hyprland-modular = {
    ...
    extraConfig = ''
      ...
      # Smart Gaps — remove bordas quando há só uma janela
      workspace = w[tv1], gapsout:0, gapsin:0
      workspace = f[1], gapsout:0, gapsin:0

      # Animações tuned para 144Hz — fluídas, sem lag visual
      animations {
        enabled = yes

        bezier = fluid,   0.25, 0.46, 0.45, 0.94
        bezier = snappy,  0.20, 0.80, 0.20, 1.00
        bezier = bounce,  0.68, -0.10, 0.26, 1.25
        bezier = linear,  0.00, 0.00, 1.00, 1.00

        animation = windowsIn,         1, 3, bounce,  slide
        animation = windowsOut,        1, 2, fluid,   popin 85%
        animation = windowsMove,       1, 3, snappy
        animation = border,            1, 6, fluid
        animation = borderangle,       1, 60, linear, loop
        animation = fade,              1, 4, fluid
        animation = fadeLayers,        1, 3, fluid
        animation = workspaces,        1, 4, fluid,   slidefade 20%
        animation = specialWorkspace,  1, 4, fluid,   slidevert
        animation = layers,            1, 3, fluid,   slide
      }
    '';
  };

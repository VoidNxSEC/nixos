...

{
  ...
  config = mkIf config.kernelcore.security.pam.enable {
    ...
    # run0 como alternativa ao sudo (systemd 256+)
    # Usar `run0 <cmd>` em vez de `sudo <cmd>` — mantém sudo disponível durante transição.
    # Quando estiver confortável, desabilitar sudo: security.sudo.enable = false
    security.polkit.enable = true;
    environment.shellAliases.r0 = "run0";
  };
}

{
  ...
  inputs = {
    ...
    # ML Offload API - Multi-backend ML orchestration (Rust/Axum)
    # Disabled: local path removed — re-enable with github URL when ready
    #ml-ops-api = {
    ...
    # mercury disabled — boot modules consolidated into main repo
    #mercury = {
      ...
      nixosConfigurations = {
        kernelcore = nixpkgs.lib.nixosSystem {
          ...
          modules = [
            ...
            #({ lib, ... }: {
            #  services.ml-ops-api.package = inputs.ml-ops-api.packages.${system}.api;
            #  systemd.services.ml-ops-api.after = lib.mkForce ["network.target" "llamacpp-swap.service"];
            #  systemd.services.ml-ops-api.wants = [ "llamacpp-swap.service" ];
            #})
            ...
            home-manager.nixosModules.home-manager
            {
              ...
              home-manager.users.kernelcore = import ./hosts/kernelcore/home/home.nix; # nx or kernelcore, I don't now
        ...
        kernelcore-iso = nixpkgs.lib.nixosSystem {
          ...
          modules = [
            ...
            # Add sops-nix module for user configurations that depend on it: .sops.yaml --> create /secrets/*.my_secret.yaml --> create modules if you want --> enable on configuration top level X. Or, store in another server.
            sops-nix.nixosModules.sops
            {
              sops.age.sshKeyPaths = [ "/etc/ssh/ssh_host_ed25519_key" ];
            }
